getBoundingCoordinates = (points) => {
  ...
  points.forEach((point) => {
    ...
    const lng2 = lng + (delta / earth_radius) * (180 / Math.PI) / Math.cos(lat * Math.PI/180);
    const lat2 = lat + (delta / earth_radius) * (180 / Math.PI);
    const lng1 = 2 * lng - Math.floor(lng2);
    const lat1 = 2 * lat - lat2;
    str += lng1 + ',' + lat1 + ';' + lng2 + ',' + lat2 +'!';
  });
...
const renderMap = (data, coordinates) => {
  ...
  var mainCoordinate = { lat: 43.2220, lng: 76.8512 }; // 52.5184443440238,13.383906494396967
...
// Create the parameters for the routing request:
  var routingParameters = {
    ...
    'waypoint0': 'geo!43.238562,76.897931', // 43.238562, 76.897931
    // The end point of the route:
    'waypoint1': 'geo!43.262367,76.950409', // 43.262367, 76.950409
  ...
  var onResult = function(result) {
    ...
    if(result.response.route) {
      ...
      data.forEach((item) => {
        ...
        let coords = {lat: item.longitude, lng: item.latitude};
